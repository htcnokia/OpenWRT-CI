...
# ---------------------------------------------------------
PKG_DIR=$(find . -maxdepth 3 -name "Makefile" | grep -i "homeproxy" | head -n 1 | xargs dirname)
if [ -n "$PKG_DIR" ]; then
  TARGET_SRS_DIR="$PKG_DIR/root/etc/homeproxy/private_srs"
else
...
if [ -f "$ts" ]; then
  sed -i "/finish 0/d" "$ts"
  echo 'rm -f "$HP_DASHBOARD/assets/ibm-plex-mono-cyrillic-400-normal-BSMlKf0J.woff2"' >> "$ts"
  echo 'mkdir -p /etc/homeproxy/private_srs' >> "$ts"
  echo 'srs_base_url="https://raw.githubusercontent.com/MetaCubeX/meta-rules-dat/sing/geo/geosite"' >> "$ts"
  echo 'for file in google.srs private.srs category-ads-all.srs cn.srs "geolocation-!cn.srs"; do' >> "$ts"
  echo '  curl -sSL "$srs_base_url/$file" -o "/etc/homeproxy/private_srs/$file"' >> "$ts"
  echo 'done' >> "$ts"
  echo 'finish 0' >> "$ts"
fi
